...
#RUTA ESTABLECIDA DE DESCARGA
if os.path.isfile(rutaInterna + "settings/RUTADW"):
    with open(rutaInterna + "settings/RUTADW", "r") as file:
        rutadw = file.readlines()[0]
else:
    rutadw = storage + "/shared/Music"
    with open(rutaInterna + "settings/RUTADW", "w") as file:
        file.write(rutadw)
...
try:
    ...
    carpeta = "."
    while carpeta == ".":
        carpeta = input("\n" + num_cols*"-" + "\nNombre de la carpeta (Salir: 's' | Ruta: '.'): ")
        if carpeta.upper() == "S":
            sys.exit()
        elif carpeta == "":
            os.system("clear")
            print(num_cols*"=")
            print("Las pistas se guardarán directamente en la ruta:\n")
            print(((num_cols - len(rutadw[47:]) - 7)//2) * " " + "'storage" + rutadw[47:] + "/'")
            print("\ndebido a que no especificaste un nombre de carpeta.")
            print(num_cols*"=")
            i = input("Estás de acuerdo (Sí: 's' | Salir: Enter)? ")
            if i.upper() != "S":
                sys.exit()
        elif carpeta == ".":
            rutadw = navegar(num_cols, rutadw, rutaInterna + "settings/RUTADW")
            os.system("clear")
            pantalla()
            print("\nIngresa la opción o URL de descarga (Salir: 's' | Ayuda: 'h'): " + link)
    ...
    while completo == False:
        ...
        coma = 'youtube-dl --ignore-errors ' + credenciales + modo + formatos[n_formato] + ' -o ' + '"' + rutadw + "/" + carpeta + '%(title)s.%(ext)s" ' + link
